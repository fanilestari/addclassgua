import { IProject } from './ProjectCard';

export const DATA_PROJECT: IProject[] = [
  {
    title: 'Murod Mutamimah',
    tag: 'Proyek pondok',
    desc: 'Projek ini dilakukan setelah menyelesaikan ngabsai dari bahasa arab ke bahasa jawa, murod artinya adalah mengartikannya menjadi bahasa indonesia agar lebih mudah dipahami ',
    img: '/img/mutamimah.jpg',
  },
  {
    title: 'Hafalan Shorof',
    tag: 'Proyek pondok',
    desc: 'Proyek ini akan sangat berguna jika sudah selesai, karena artinya kita aku bisa mengharokati dan membaca kitab gundul tanpa melihat tashrifannya di buku sorof',
    img: '/img/sorof.jpeg',
  },
  {
    title: 'Hafalan Imrithy',
    tag: 'Proyek pondok',
    desc: 'supaya mengetahui kaidah nahwu',
    img: '/img/imriti.jpg',
  },
  
];
